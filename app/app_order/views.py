...
def solicitar_os(request):
    ...
    etapas_ativas = Stage.objects.filter(tecnico=request.user, status="em_execucao", order__status="em_andamento").count()
    form = OrdemServicoForm(request.POST or None)
    if request.method == "POST":
        if os_pendentes or etapas_ativas >= MAX_ETAPAS_EM_ANDAMENTO:
            return render(request, "app_order/solicitar_os.html", {"form": form, "os_pendentes": os_pendentes, "etapas_ativas": etapas_ativas})
        if form.is_valid():
...
def painel_admin(request):
    ...
    context = {
        ...
        "novas_24h": OrdemServico.objects.filter(data_solicitacao__gte=timezone.now() - datetime.timedelta(hours=24)).count(),
...
def user_list(request):
    ...
    return render(request, "app_order/admin_user_list.html", {"users": users})
...
def notificacoes_os(request):
    return JsonResponse({"count": OrdemServico.objects.filter(status="aguardando").count()})
...
def user_edit(request, user_id):
    ...
    user_data = {"id": user.id, "username": user.username, "email": user.email, "is_active": user.is_active, "groups": list(user.groups.values_list("id", flat=True))}
...
def listar_os_funcionario(request):
    etapas = Stage.objects.filter(tecnico=request.user, status="em_execucao", order__status="em_andamento")
    criadas = OrdemServico.objects.filter(usuario=request.user).order_by("-data_solicitacao")
    contribui = OrdemServico.objects.filter(stages__tecnico=request.user).exclude(usuario=request.user).distinct().order_by("-data_solicitacao")
    form = StageActionForm(user=request.user)
    now = timezone.localtime()
    return render(request, "app_order/listar_os_funcionario.html", {"etapas": etapas, "criadas": criadas, "contribui": contribui, "form": form, "now": now})
        ...
        with transaction.atomic():
            ...
            if pd.get("repassar_para"):
                if total < ordem.repass_limite:
                    Stage.objects.create(order=ordem, tecnico=pd["repassar_para"], ordem=total + 1)
                    messages.success(request, f"Ordem repassada para {pd['repassar_para'].username} com sucesso.")
                else:
...
def verificar_numero_os(request, pk):
    ...
    return JsonResponse({"numero_os": ordem.numero_os or None, "status": ordem.status})
...
def detalhes_os(request, os_id):
    qs = OrdemServico.objects.filter(Q(usuario=request.user) | Q(stages__technik=request.user)).distinct()
    ordem = get_object_or_404(qs, pk=os_id)
    etapas = ordem.stages.all().order_by("ordem", "criado_em")
    return render(request, "app_order/partials/timeline.html", {"etapas": etapas, "now": timezone.localtime()})
